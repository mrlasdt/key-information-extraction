"""
cannot word because:
    - just a search engine, not a corrector at the beginning
        => need human decision
        e.g. search quận hoa -> quan hoa cầu giấy
    - the ocr error are complicated:
        - missing comma
            => while loop to search
        - missing vietnam, province...
            => have to lookup many place
            => if error on district -> totally lost

        - pre + name or pre: e.g Tỉnh Bắc Ninh -> Băc Ninh mached, what about Tỉnh?
            => cannot correct error on pre
            !!! cannot match Tinh Bac Ninh with current algorithm even Tinh Bac Ninh is in the database because Bac Ninh will match first
            => put 3 before 2 may solve the problem
        
        - unexpected match -> Thanh pho Hue, Phuong 03 -> Phuong 13
        - cannot correct error on street name
...
import unicodedata
import pickle
DATA_DIR = Path('/home/sds/hungbnt/web_crawler/vietnam_dataset/data/')
