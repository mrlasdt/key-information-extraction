...
def load_train_val_manually(train_root, val_root):
    train_paths = glob.glob(os.path.join(train_root, "*.jpg"))
    train_labels = [path[:-3] + "txt" for path in train_paths]
    train_df = pd.DataFrame.from_dict({'image_path': train_paths, 'label': train_labels})

    val_paths = glob.glob(os.path.join(val_root, "*.jpg"))
    val_labels = [path[:-3] + "txt" for path in val_paths]
    val_df = pd.DataFrame.from_dict({'image_path': val_paths, 'label': val_labels})
...
def load_data(train_path, val_path, max_seq_len, batch_size, pretrained_processor, kie_labels, device):
    train_df, val_df = load_train_val_manually(train_path, val_path)
    ...
    processor = AutoProcessor.from_pretrained(pretrained_processor, apply_ocr=False)
